...
require_once 'phpsdk/src/Class_ServerClub_SMS_API.php';
use ServerClubSMS\ServerClubSMSAPI;

class ServerClubDigitalTrigger
{
    /*
     * Declare setting prefix and other values.
     */

    private $prefix = 'serverclubdigital_sms_woo_';
    private $ApiKey, $sendId, $adminRecipients;
    private $yesAdminMsg;
    ...
    {
        /*
         * Get Serverclub.Digital SMS configuration settings.
         */
        ...
        add_action('woocommerce_order_status_changed', array($this, 'serverclubdigi_send_sms_for_events'), 11, 3);
        add_action('woocommerce_new_customer_note', array($this, 'serverclubSendOrderNoteSMS'));
    }

    public function serverclubdigi_send_sms_for_events($order_id, $from_status, $to_status)
    {
        if (get_option($this->prefix . 'send_sms_' . $to_status) !== "yes")
            return;
        $this->ServerclubSMSsend($order_id, $to_status);
    }

    public function serverclubdigi_send_admin_sms_for_woo_new_order($order_id)
    {
        if ($this->yesAdminMsg)
            $this->ServerclubSMSsend($order_id, 'admin-order');
    }

    public function serverclubSendOrderNoteSMS($data)
    {
        if (get_option($this->prefix . 'enable_notes_sms') !== "yes")
            return;

        $this->ServerclubSMSsend($data['order_id'], 'new-note', $data['customer_note']);
    }
    ...
    private function ServerclubSMSsend($order_id, $status, $message_text = '')
    {
        ...
        foreach ($to_numbers as $numb) {
            ...
            try {
                $client->send_sms($sms_body, $url, $fName, $lName, $bEmail, $address);
            } catch (\Throwable $th) {
                //throw $th;
            }
        }
